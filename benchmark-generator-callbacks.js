...
module.exports = {
	benchmark: benchmarkGeneratorCallbacks,
};

function benchmarkGeneratorCallbacks(count, callback) {
...
if (process.env.RUN) {
	...
	benchmarkGeneratorCallbacks(count, (err, times) => {
		out = !times ? undefined : {
			elapsed: times.elapsed,
			track: times.track,
		};
		console.log('benchmarkGeneratorCallbacks', err, out);
	});
}
